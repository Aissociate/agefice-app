...
import { AlertTriangle, TrendingUp, FolderOpen, CheckCircle, Clock, Users, Target } from "lucide-react";
...
async function getDashboardData() {
  ...
  const [
    totalDossiers, allDossiers, urgentDossiers, recentDossiers, remboursesDossiers,
    leadsStats, dossiersMonth,
  ] = await Promise.all([
    ...
    // Stats leads pipeline
    prisma.lead.groupBy({ by: ["statut"], _count: true }),
    // Dossiers créés ce mois
    prisma.dossier.count({ where: { dateCreation: { gte: startOfMonth } } }),
  ]);

  const dossiersByStatut = Object.fromEntries(allDossiers.map((d) => [d.statut, d._count]));
  const leadsByStatut = Object.fromEntries(leadsStats.map((l) => [l.statut, l._count]));
...
function DossierRow({ dossier }: { dossier: any }) {
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors">
      ...
      <td className="px-4 py-3 text-sm text-gray-900">{dossier.client?.prenom} {dossier.client?.nom}</td>
      <td className="px-4 py-3 text-sm text-gray-600 max-w-xs truncate">{dossier.formation?.intitule}</td>
      <td className="px-4 py-3 text-sm text-gray-600">{dateLimite ? formatDate(dateLimite) : "—"}</td>
...
export default async function DashboardPage() {
  ...
  const nouveauxLeads = data.leadsByStatut?.nouveau ?? 0;
  const dossiersAgefice = data.leadsByStatut?.qualifie ?? 0;
  const inscrits = data.leadsByStatut?.inscrit ?? 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* KPIs dossiers */}
      <div>
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Dossiers AGEFICE</p>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard title="Remboursés (année)"  value={data.rembourseCount}             icon={CheckCircle} color="bg-green-600" sub={`${data.dossiersMonth} ce mois`} />
          <StatCard title="CA remboursé"        value={formatMontant(data.chiffreAffaires)} icon={TrendingUp} color="bg-[#E4620D]" />
        </div>
      </div>

      {/* KPIs pipeline prospection */}
      <div>
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Pipeline prospection</p>
        <div className="grid grid-cols-3 gap-4">
          <StatCard title="Nouveaux leads"        value={nouveauxLeads}   icon={Users}       color="bg-blue-600"   sub="En attente de contact" />
          <StatCard title="Dossiers en cours"     value={dossiersAgefice} icon={FolderOpen}  color="bg-green-600"  sub="Qualifiés AGEFICE" />
          <StatCard title="Inscrits à la formation" value={inscrits}      icon={Target}      color="bg-purple-600" sub="Formation confirmée" />
        </div>
      </div>

      {/* Graphiques */}
      <div>
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Analyses & visualisations</p>
        <DashboardCharts />
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <Link href="/dossiers" className="text-sm text-blue-600 hover:text-blue-800 font-medium">Voir tous →</Link>
        </div>
        <div className="overflow-x-auto">
          {data.recentDossiers?.length === 0 ? (
            <div className="px-5 py-10 text-center text-gray-400 text-sm">
              Aucun dossier créé pour le moment.{" "}
              <Link href="/dossiers/nouveau" className="text-blue-600 hover:underline">Créer un premier dossier</Link>
            </div>
          ) : (
            <table className="w-full">
              <thead>
